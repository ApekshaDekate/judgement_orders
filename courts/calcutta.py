...
CALCUTTA_BENCHES = {
    "original": {
        "label": "Original Side",
        "court_code": "1",
        "form_url": "https://hcservices.ecourts.gov.in/ecourtindiaHC/index_highcourt.php?state_cd=16&dist_cd=1&stateNm=Calcutta"
    },
    "jalpaiguri": {
        "label": "Circuit Bench At Jalpaiguri",
        "court_code": "2",
        "form_url": "https://hcservices.ecourts.gov.in/ecourtindiaHC/index_highcourt.php?state_cd=16&dist_cd=1&court_code=2&stateNm=Calcutta"
    },
    "appellate": {
        "label": "Appellate Side",
        "court_code": "3",
        "form_url": "https://hcservices.ecourts.gov.in/ecourtindiaHC/index_highcourt.php?state_cd=16&dist_cd=1&court_code=3&stateNm=Calcutta"
    },
    "port_blair": {
        "label": "Circuit Bench At Port Blair",
        "court_code": "4",
        "form_url": "https://hcservices.ecourts.gov.in/ecourtindiaHC/index_highcourt.php?state_cd=16&dist_cd=1&court_code=4&stateNm=Calcutta"
    },
}

# ----------------- Utilities (same robust approach as Andhra) -----------------
def fetch_calcutta_judges(court_code: str):
    """
    Fetch judge list for a given calcutta bench (court_code).
    Same behaviour as your Andhra judge dropdown function.
    """
    url = f"{CALCUTTA_BASE_URL}s_order_qry.php"
    ...
    try:
        ...
        print("Error fetching calcutta judges:", e)
        return []
...
def get_captcha(session: requests.Session) -> str:
    ...
    try:
        r = session.get(CALCUTTA_CAPTCHA_URL, verify=False, timeout=15)
        img_bytes = r.content
...
def download_calcutta_display(display_url: str, save_path: Path, referer_form_url: str = None) -> bool:
    """
...
# ----------------- Routes -----------------
@router.get("/calcutta", response_class=HTMLResponse)
async def calcutta_form(request: Request):
    benches = [{"id": k, "label": v["label"]} for k, v in CALCUTTA_BENCHES.items()]

    # # load judges for default bench (guwahati)
    # default_bench = CALCUTTA_BENCHES["guwahati"]
    # judge_list = fetch_calcutta_judges(default_bench["court_code"])
...
@router.get("/calcutta/judges/{bench_id}")
async def get_calcutta_judges(bench_id: str):
    bench = CALCUTTA_BENCHES.get(bench_id)
    if not bench:
        return {"judges": []}
    judges = fetch_calcutta_judges(bench["court_code"])
    return {"judges": judges}


@router.post("/calcutta/results", response_class=HTMLResponse)
async def fetch_calcutta_results(request: Request):
    ...
    if search_type == "judge" and judge_code:
        ...
        post_url = f"{CALCUTTA_BASE_URL}s_order_qry.php"
        payload = {
            "__csrf_magic": "",
        ...
        post_url = f"{CALCUTTA_BASE_URL}s_partyorder_qry.php"
        payload = {
            "__csrf_magic": "",
            "action_code": "showRecords",
            "state_code": "16",
        ...
        post_url = f"{CALCUTTA_BASE_URL}s_citation_qry.php"
        payload = {
            "__csrf_magic": "",
            "action_code": "showRecords",
            "state_code": "16",
        ...
        post_url = f"{CALCUTTA_BASE_URL}s_orderdate_qry.php"
        payload = {
            "__csrf_magic": "",
            "action_code": "showRecords",
            "state_code": "16",
    ...
    for idx, rec in enumerate(records, start=1):
        ...
        if not save_path.exists():
            downloaded = download_calcutta_display(display_url, save_path, referer_form_url=form_url)
        else:
            downloaded = True
    ...
    # render results page
    rendered_html = templates.get_template("calcutta_results.html").render(request=request, data=data, bench=bench)
